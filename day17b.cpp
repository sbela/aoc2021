...
int main()
{
    ...
    std::vector<Point> initV;
    for (int vx = 1; vx <= sample_target.BottomRight.x; ++vx)
    {
        for (int vy = sample_target.BottomRight.y; vy <= sample_target.BottomRight.x; ++vy)
        {
            int highestY{0};
            ...
            for (int step = 0; step <= max_step; ++step)
            {
                ...
                if (isInTarget(probe, sample_target))
                {
                    ...
                    initV.emplace_back(Point{vx, vy});
                    break;
                }
    ...
    std::cout << "All Time high: " << allTimeHighestY << " # " << initV.size() << std::endl;
    return 0;
}