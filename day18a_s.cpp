...
std::ostream &operator<<(std::ostream &o, const std::vector<Num> &s)
{
    if (not s.size())
        return o;
...
struct SnailFish
{
    ...
    SnailFish() = default;
    explicit SnailFish(std::string_view s)
    {
        int current_depth = 0;
        for (auto c : s)
        {
    ...
    SnailFish operator+(const SnailFish &other) const
    {
        ...
        int add = pass1(number | std::views::transform([](auto n)
                                                       { return Num{n.value, n.depth + 1}; }));
        //std::cout << pass1_result << std::endl;
        pass1(other.number | std::views::transform([](auto n)
                                                   { return Num{n.value, n.depth + 1}; }),
              add);
...
std::string_view input = R"([[[0,[4,5]],[0,0]],[[[4,5],[2,6]],[9,5]]]
[7,[[[3,7],[4,3]],[[6,3],[8,8]]]])";

/*std::string_view input = R"([[4,[3,[2,8]]],[[[8,5],4],[[3,5],[4,9]]]]
[[[8,[6,8]],2],[4,[[1,0],[4,5]]]]
[[4,3],[[[3,3],[1,8]],[[0,8],[5,8]]]]
